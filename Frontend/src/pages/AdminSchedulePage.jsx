...
import { fetchActivityOccurrences, fetchActivities, fetchZones, createActivityOccurrence, updateActivityOccurrence } from "../services/api";
...
export default function AdminSchedulePage() {
	...
	useEffect(() => { loadData(); }, [selectedYear, selectedWeek]);
	...
	async function loadData() {
		try {
			...
			setZones(Array.isArray(zns) ? zns : []);
				try {
					console.debug("Schedule load:", { occurrences: occ?.length, activities: acts?.length, zones: zns?.length });
				} catch {}
		} catch (e) {
			console.error(e);
			...
			<div className="admin-schedule">
				<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
					...
					<div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
						<select value={selectedYear} onChange={e => setSelectedYear(Number(e.target.value))} style={{ fontSize: 15, padding: '2px 8px' }}>
							{yearOptions.map(y => <option key={y} value={y}>{y}</option>)}
						</select>
						<button onClick={handlePrevWeek} style={{ fontSize: 18, padding: '2px 8px' }}>&lt;</button>
						<span style={{ fontWeight: 600, minWidth: 36, textAlign: 'center' }}>{`v${selectedWeek}`}</span>
						<button onClick={handleNextWeek} style={{ fontSize: 18, padding: '2px 8px' }}>&gt;</button>
						<select value={selectedWeek} onChange={e => setSelectedWeek(Number(e.target.value))} style={{ fontSize: 15, padding: '2px 8px' }}>
							{weekOptions.map(w => <option key={w} value={w}>{`v${w}`}</option>)}
						</select>
					</div>
				</div>
				...
				<div className="grid-body" style={{ gridTemplateRows: `repeat(${hours.length}, ${ROW_HEIGHT}px)`, gridTemplateColumns: `80px repeat(7, 1fr)` }}>
					{hours.map((h, rowIdx) => (
						<React.Fragment key={`row-${rowIdx}`}>
							<div className="time-col" style={{ gridColumn: 1, gridRow: rowIdx + 1 }}>
								{`${String(h).padStart(2, "0")}:00`}
											...
											{weekDays.map((d, dayIdx) => (
												<div
													...
													style={{ gridColumn: dayIdx + 2, gridRow: rowIdx + 1, position: 'relative', padding: 0 }}
												>
													{[0,15,30,45].map((m, qi) => (
														<button
															key={`q-${qi}`}
															className={"slot-quarter"}
															title={`Lägg till ${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`}
									...
									const dayOccs = occurrences.filter(o => {
										...
										return st.getFullYear() === dY && st.getMonth() === dM && st.getDate() === dD;
									});
												try { console.debug(`Day ${dayIdx} ${d.toDateString()} occurrences:`, dayOccs.length); } catch {}
						return (
							<div
								key={`overlay-${dayIdx}`}
								className="day-overlay"
								style={{ gridColumn: dayIdx + 2, gridRow: `1 / ${hours.length + 1}`, position: 'relative', zIndex: 5, pointerEvents: 'none' }}
							>
												{dayOccs.map(o => {
													const st = new Date(o.startTime);
													// Fallback: compute end from duration if API doesn't provide endTime
													const en = o.endTime ? new Date(o.endTime) : new Date(st.getTime() + (Number(o.durationMinutes) || 60) * 60000);
													// Always use the day's visible start (e.g. 08:00)
													const visStart = new Date(d); visStart.setHours(startHour, 0, 0, 0);
													...
													if (!isFinite(durationMin) || durationMin <= 0) durationMin = Number(o.durationMinutes) || 60;
													const top = (minutesFromStart / 60) * ROW_HEIGHT;
													const height = (durationMin / 60) * ROW_HEIGHT;
													const startLabel = `${String(st.getHours()).padStart(2,'0')}:${String(st.getMinutes()).padStart(2,'0')}`;
													const endLabel = `${String(en.getHours()).padStart(2,'0')}:${String(en.getMinutes()).padStart(2,'0')}`;
													return (
														<button
															key={`occ-${o.id}`}
															className="occurrence overlay show"
															style={{ position: 'absolute', top: `${top}px`, left: 0, right: 0, height: `${height}px`, zIndex: 10, pointerEvents: 'auto', opacity: 1, paddingRight: '56px' }}
															onClick={() => openEdit(o)}
															title={`${startLabel}–${endLabel} ${activityMap[o.activityId]?.name || o.activityName || "Aktivitet"}`}
														>
															<span style={{ pointerEvents: 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
																{o.activityName || activityMap[o.activityId]?.name || ""}
															</span>
															<span style={{ position: 'absolute', top: '2px', right: '6px', fontSize: '11px', color: '#065f46', opacity: 0.9, pointerEvents: 'none' }}>
																{startLabel}–{endLabel}
															</span>
														</button>
													);
...
function OccurrenceModal({ onClose, onSave, editing, slotInfo, activities, occurrences, zones }) {
	const [activityId, setActivityId] = useState(editing?.activityId || (activities[0]?.id || ""));
	const [dateStr, setDateStr] = useState(() => {
		const d = editing ? new Date(editing.startTime) : new Date(slotInfo?.date || new Date());
	...
	const [durationMinutes, setDurationMinutes] = useState(editing?.durationMinutes || 60);
		...
		<div className="modal-backdrop" role="dialog" aria-modal="true">
			<div className="modal">
				<div className="modal-header">
					<h2>{editing ? "Redigera aktivitetstillfälle" : "Nytt aktivitetstillfälle"}</h2>
				...
				<form onSubmit={handleSubmit} className="modal-body">
					...
					<fieldset className="inline-fields">
						...
						<label>
							...
							<select value={timeStr} onChange={(e) => setTimeStr(e.target.value)}>
								{getAvailableTimes({ dateStr, zoneId: computedZoneId, durationMinutes, occurrences, editingId: editing?.id }).map(t => (
					...
					<div className="duration-choices">
						...
						<div className="choices">
							{[30,45,60,75,90,105,120].map(min => (
								<span
									key={min}
									className={`choice ${Number(durationMinutes) === min ? 'selected' : ''}`}
					...
					<div className="modal-footer">
						...
						<button type="submit" className="btn-primary" disabled={!activityId || !dateStr || !timeStr || !durationMinutes}>Spara</button>
					</div>
				</form>
			</div>
		</div>
...
function getAvailableTimes({ dateStr, zoneId, durationMinutes, occurrences, editingId }) {
	...
	for (let h = startHour; h <= endHour; h++) {
		for (let m = 0; m < 60; m += 15) {
			...
			slots.push({ s, e, label: `${String(h).padStart(2,'0')}.${String(m).padStart(2,'0')}` });
		}
	}
	...
	const sameDayOccs = (occurrences || []).filter(o => {
		...
		const st = new Date(o.startTime);
		return st.getFullYear() === startOfDay.getFullYear()
